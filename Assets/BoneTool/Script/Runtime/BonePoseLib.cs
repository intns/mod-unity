...
namespace BoneTool.Script.Runtime
{
    ...
    public class BonePoseLib : MonoBehaviour
    {
        ...
        public void SetPose(int index)
        {
            ...
            ArmaturePose pose = Poses[index];
            pose.BonePoses = new List<BonePose>();

            Transform[] nodes = GetComponent<BoneVisualiser>().GetChildNodes();
...
#endif
            foreach (BonePose bonePose in pose.BonePoses)
            {
                bonePose.Apply();
            }
        }
