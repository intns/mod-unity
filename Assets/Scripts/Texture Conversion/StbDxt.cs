    ...
    public static class StbDxt
    {
        ...
        static void DitherBlock(byte[] dest, int destIdx, byte[] block, int blockIdx)
        {
            ...
            for (ch = 0; ch < 3; ++ch)
            {
                ...
                {
                    dest[dp + 0] = quant[
                        8 + block[bp + 0] + ((3 * err[ep2 + 1] + 5 * err[ep2 + 0]) >> 4)
                    ];
                    ...
                    dest[dp + 12] = quant[
                        8
                            + block[bp + 12]
                            + ((7 * err[ep1 + 2] + 5 * err[ep2 + 3] + err[ep2 + 2]) >> 4)
                    ];
        ...
        )
        {
            ...
            for (i = 0; i < 16; i++)
                dots[i] =
                    block[blockIdx + i * 4 + 0] * dirr
                    + block[blockIdx + i * 4 + 1] * dirg
                    + block[blockIdx + i * 4 + 2] * dirb;

            for (i = 0; i < 4; i++)
                stops[i] =
                    color[colorIdx + i * 4 + 0] * dirr
                    + color[colorIdx + i * 4 + 1] * dirg
                    + color[colorIdx + i * 4 + 2] * dirb;
        ...
        static void OptimizeColorsBlock(
            byte[] block,
            int blockIdx,
            out ushort pmax16,
            out ushort pmin16
        )
        {
            int mind = 0x7fffffff,
                maxd = -0x7fffffff;
            int minp = 0,
                maxp = 0; // Give them a value to avoid "unassigned variable warning"
            double magn;
            int v_r,
                v_g,
                v_b;
            ...
            {
                int dot =
                    block[blockIdx + i * 4 + 0] * v_r
                    + block[blockIdx + i * 4 + 1] * v_g
                    + block[blockIdx + i * 4 + 2] * v_b;
        ...
        static void CompressColorBlock(
            byte[] dest,
            int destIdx,
            byte[] block,
            int blockIdx,
        ...
        )
        {
            ...
            for (i = 1; i < 16; i++)
                if (
                    block[blockIdx + i * 4 + 0] != block[blockIdx + 0]
                    || block[blockIdx + i * 4 + 1] != block[blockIdx + 1]
                    || block[blockIdx + i * 4 + 2] != block[blockIdx + 2]
                    || block[blockIdx + i * 4 + 3] != block[blockIdx + 3]
                )
                    break;

            if (i == 16)
            { // constant color
                int r = block[blockIdx + 0],
                    g = block[blockIdx + 1],
                    b = block[blockIdx + 2];
                mask = 0xaaaaaaaa;
                max16 = (ushort)((OMatch5[r, 0] << 11) | (OMatch6[g, 0] << 5) | OMatch5[b, 0]);
        ...
        /// </summary>
        static void CompressAlphaBlock(
            byte[] dest,
            int destIdx,
            byte[] src,
            int srcIdx,
        ...
        )
        {
            ...
            for (i = 1; i < 16; i++)
            {
                if (src[srcIdx + i * 4 + 3] < mn)
                    mn = src[srcIdx + i * 4 + 3];
                else if (src[srcIdx + i * 4 + 3] > mx)
                    mx = src[srcIdx + i * 4 + 3];
            }
        ...
        public static void CompressDxtBlock(
            byte[] dest,
            int destIdx,
            byte[] src,
            int srcIdx,
