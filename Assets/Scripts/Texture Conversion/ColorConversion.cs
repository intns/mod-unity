...
namespace LibGC.Texture
{
    ...
    static class ColorConversion
    {
        ...
        public static ColorRGBA IA8ToColor(ushort v)
        {
            int i = v >> 8;
            int a = v & 0xFF;

            return new ColorRGBA((byte)i, (byte)(i >> 8), (byte)(i >> 16), (byte)a);
        }

        public static ushort ColorToIA8(ColorRGBA color)
        {
            ushort i = color.Red; // Assuming R, G, and B are the same
            ushort a = color.Alpha;
        ...
        {
            return (ushort)((Bits8To5[c.Red] << 11) | (Bits8To6[c.Green] << 5) | Bits8To5[c.Blue]);
        }
        ...
        public static ushort ColorToRGB5A3(ColorRGBA c)
        {
            if (c.Alpha >= 0xE0)
            {
                // RGB555
                return (ushort)(
                    0x8000 | (Bits8To5[c.Red] << 10) | (Bits8To5[c.Green] << 5) | Bits8To5[c.Blue]
                );
            }
            else
            {
