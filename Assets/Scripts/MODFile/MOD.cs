    ...
    public class MOD
    {
        ...
        public void Read(BinaryReader reader)
        {
            ...
            while (!stopRead)
            {
                ...
                Debug.Assert(
                    position % 0x20 == 0,
                    $"Error in chunk {opcode}, offset {position}, chunk start isn't aligned to 0x20, this means an improper read occurred."
                );
                Debug.Assert(iterations++ <= 100, "Error in chunk, too many iterations");

                // Debug.Log(
                //     $"Reading 0x{opcode:X}, {GetChunkName(opcode) ?? "Unknown chunk"} at 0x{position:X}"
                // );
#endif
