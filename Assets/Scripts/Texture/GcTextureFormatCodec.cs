...
namespace LibGC.Texture
{
    ...
    public abstract class GcTextureFormatCodec
    {
        ...
        protected abstract void DecodeTile(
            byte[] dst,
            int dstPos,
            int stride,
            byte[] src,
        ...
        )
        {
            ...
            if (stride < width * 4)
                throw new ArgumentOutOfRangeException(
                    "stride",
                    "Stride is too small to contain a row of data."
                );
            if (src == null)
                    ...
                    else
                    {
                        ...
                        for (int ty = 0; ty < Math.Min(height - y, tileHeight); ty++)
                        {
                            Array.Copy(
                                ...
                                ty * tileWidth * 4,
                                dst,
                                dstPos + x * 4 + (y + ty) * stride,
                                Math.Min(width - x, tileWidth) * 4
                            );
                        }
                    }
                    srcPos += tileSize;
        ...
        static void DistributeDitheringError(
            byte[] src,
            int srcPos,
            int width,
            int height,
        ...
        )
        {
            if ((x + deltaX) < width && (y + deltaY) < height)
            {
                ...
                ColorRGBA newPx =
                    new(
                        Utils.Trim8((int)(origPx.Red + errorR * weight)),
        ...
        static void GrayscaleImage(byte[] src, int srcPos, int width, int height, int stride)
        {
            ...
            for (int y = 0; y < height; y++, pos += stride - width * 4)
            {
                for (int x = 0; x < width; x++, pos += 4)
                {
                    ...
                    ColorRGBA grayscaleColor = ColorRGBA.FromIntensityAlpha(
                        srcColor.Intensity(),
                        srcColor.Alpha
                    );
        ...
        /// <param name="dstPos">Initial position in the destination buffer.</param>
        protected abstract void EncodeTile(
            byte[] src,
            int srcPos,
            int stride,
            byte[] dst,
        ...
        )
        {
            ...
            if (srcPos + height * stride > src.Length) // Image doesn't fit in source buffer
                throw new ArgumentOutOfRangeException(
                    "src",
                    "src is too small to contain an image with the given parameters."
                );
            if (width < 0)
                throw new ArgumentOutOfRangeException("width");
            if (height < 0)
            ...
            if (stride < width * 4)
                throw new ArgumentOutOfRangeException(
                    "stride",
                    "Stride is too small to contain a row of input data."
                );
            if (dst == null)
                throw new ArgumentNullException("dst");
            if (dstPos < 0)
                ...
                for (int x = 0; x < width; x += tileWidth)
                {
                    ...
                    else
                    {
                        ...
                        for (int ty = 0; ty < Math.Min(height - y, tileHeight); ty++)
                        {
                            Array.Copy(
                                processedSrc,
                                processedSrcPos + x * 4 + (y + ty) * stride,
                                tileTmp,
                                ty * tileWidth * 4,
                                Math.Min(width - x, tileWidth) * 4
                            );
        ...
        public static GcTextureFormatCodec GetCodec(GcTextureFormat palFmt)
        {
            switch (palFmt)
            {
                case GcTextureFormat.I4:
                    return new GcTextureFormatCodecI4();
                case GcTextureFormat.I8:
                    return new GcTextureFormatCodecI8();
                case GcTextureFormat.IA4:
                    return new GcTextureFormatCodecIA4();
                case GcTextureFormat.IA8:
                    return new GcTextureFormatCodecIA8();
                case GcTextureFormat.RGB565:
                    return new GcTextureFormatCodecRGB565();
                case GcTextureFormat.RGB5A3:
                    return new GcTextureFormatCodecRGB5A3();
                case GcTextureFormat.RGBA8:
                    return new GcTextureFormatCodecRGBA8();
                case GcTextureFormat.CI4:
                    return new GcTextureFormatCodecCI4();
                case GcTextureFormat.CI8:
                    return new GcTextureFormatCodecCI8();
                case GcTextureFormat.CI14X2:
                    return new GcTextureFormatCodecCI14X2();
                case GcTextureFormat.CMPR:
                    return new GcTextureFormatCodecCMPR();
                default:
